...
- `window.showSportReadinessCheck`, `window.selectSportReadiness`, `window.cancelSportReadinessCheck`, and `window.getSportCheckPromptSnapshot` remain compatibility delegates, but typed prompt ownership now lives in `src/app/services/workout-ui-actions.ts` instead of the legacy workout layer.
...
- `window.finishWorkout` and `window.cancelWorkout` remain compatibility delegates, but typed workout lifecycle ownership now lives in `src/stores/workout-store.ts`; legacy `applyWorkoutTeardownPlan` remains as the DOM shell teardown helper.
- `window.addExerciseByName` remains a compatibility delegate, but active-workout add-exercise mutation ownership now lives in `src/stores/workout-store.ts`.
- `window.swapAuxExercise` and `window.swapBackExercise` remain compatibility delegates, but active-workout swap mutation ownership now lives in `src/stores/workout-store.ts`; typed catalog selection still supplies the chosen exercise.
- `window.openExerciseCatalogForAdd`, `window.openExerciseCatalogForSwap`, `window.openExerciseCatalogForSettings`, `window.setExerciseCatalogSearch`, `window.setExerciseCatalogFilter`, `window.clearExerciseCatalogFilters`, `window.resetExerciseCatalogState`, `window.submitExerciseCatalogSelection`, and `window.selectExerciseCatalogExercise` remain compatibility delegates, but typed exercise catalog open/search/filter/select ownership now lives in `src/app/services/exercise-catalog.ts`.
...
- `window.applyQuickWorkoutAdjustment` and `window.undoQuickWorkoutAdjustment` remain compatibility delegates, but typed quick workout adjustment mutation ownership now lives in `src/stores/workout-store.ts`; legacy workout UI still owns the shorten-options modal shell.
- `window.openExerciseGuide`, `window.closeExerciseGuide`, `window.getExerciseGuidePromptSnapshot`, and `window.refreshExerciseGuideModal` remain compatibility delegates, but typed exercise-guide overlay ownership now lives in `src/app/services/workout-ui-actions.ts`.
