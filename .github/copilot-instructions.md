...
- `window.addExerciseByName` remains a compatibility delegate, but active-workout add-exercise mutation ownership now lives in `src/stores/workout-store.ts`; legacy catalog code still owns search/filter/open/select state until that UI state is migrated.
