...
- `window.cancelWorkout` remains a compatibility delegate, but typed cancel teardown ownership now lives in `src/stores/workout-store.ts`; legacy `applyWorkoutTeardownPlan` remains shared by the still-legacy finish flow.
