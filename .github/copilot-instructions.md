...
- The current shell-replacement step runs through the shared React app entry in `src/app/main.tsx`, which mounts the shell from `src/app/AppShell.jsx`: visible bottom navigation, toast host, confirm modal, exercise catalog/name modal, workout overlay hosts, settings overlay hosts, onboarding, and the top-level page container (rendered into the existing content root via portal). `core/ui-shell.js` remains the compatibility bridge for `showPage(...)`, `showToast(...)`, and `showConfirm(...)`, but those wrappers now push into React-owned Zustand store actions instead of React polling shell snapshot getters or DOM state. The mounted React shell owns nav metadata, confirm/toast rendering, active-page rendering, content scroll-lock state, and page-activation timing after route changes.
