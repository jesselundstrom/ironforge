...
- The first shell-replacement step runs the visible bottom navigation, toast host, and confirm modal through `src/app-shell/main.jsx`; the exercise catalog/name modal host through `src/name-modal-shell/main.jsx`; and the workout overlay hosts through `src/workout-overlays-shell/main.jsx`, while `core/ui-shell.js` remains the compatibility bridge for `showPage(...)`, `showToast(...)`, `showConfirm(...)`, page activation side effects, and legacy DOM ids.
