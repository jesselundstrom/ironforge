...
function renderDashboardTodayPlan(input) {
  ...
  const bestDaysText = bestDays.length
    ...
    : highlightDashboardText(
        (coachingInsights.frictionItems || [])[0] || '',
        []
      );
  const stats = [
    {
  ...
  const insights = [
    ...
    {
      tone:
        progressMetric.color === 'green'
          ? 'green'
          : progressMetric.color === 'red'
            ? 'red'
            : 'blue',
      text:
        progressionText ||
        sanitizeDashboardRichText(coachingInsights.progressionSummary || ''),
    },
        ...
        stats: [
          {
            ...
            label: trDash('dashboard.insights.adherence', '30 pv toteuma'),
            color:
              Math.max(0, Math.round(coachingInsights?.adherenceRate30 || 0)) >=
              50
                ? 'orange'
                : 'text',
          },
