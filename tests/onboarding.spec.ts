import { expect, test, type Page } from '@playwright/test';
import { openApp } from './helpers';

async function bootSignedInApp(page: Page) {
  await page.waitForFunction(() => typeof window.loadData === 'function');

  await page.evaluate(() => {
    const runtimeWindow = window as Window & {
      __IRONFORGE_SUPABASE__?: {
        auth?: {
          getSession?: () => Promise<unknown>;
        };
      };
    };

    const suppressLoginUi = () => {
      document.body.classList.remove('login-active');
      const loginScreen = document.getElementById('login-screen');
      if (loginScreen) loginScreen.style.display = 'none';
    };

    window.showLoginScreen = suppressLoginUi;
    window.hideLoginScreen = suppressLoginUi;
    window.__IRONFORGE_SET_AUTH_STATE__?.({
      phase: 'signed_in',
      isLoggedIn: true,
      pendingAction: null,
      message: '',
      messageTone: '',
    });
    currentUser = {
      id: window.__IRONFORGE_TEST_USER_ID__ || 'e2e-user',
      email: 'e2e@example.com',
    };

    if (runtimeWindow.__IRONFORGE_SUPABASE__?.auth) {
      runtimeWindow.__IRONFORGE_SUPABASE__.auth.getSession = async () => ({
        data: {
          session: {
            access_token: 'test-access-token',
            user: currentUser,
          },
        },
        error: null,
      });
    }

    suppressLoginUi();
  });

  await page.evaluate(async () => {
    const loadData = window.loadData;
    if (typeof loadData !== 'function') {
      throw new Error('loadData is not available on window');
    }

    await loadData({
      allowCloudSync: false,
      userId: window.__IRONFORGE_TEST_USER_ID__ || 'e2e-user',
    });
  });

  await page.waitForFunction(() => {
    if (window.__IRONFORGE_APP_SHELL_READY__ === true) return true;
    const root = document.getElementById('app-shell-react-root');
    return !!root && root.children.length > 0;
  });
}

test('onboarding auto-opens only once and then stays settings-only', async ({
  page,
}) => {
  await openApp(page);
  await page.evaluate(() => {
    localStorage.clear();
    sessionStorage.clear();
    window.resetRuntimeState?.();
  });
  await bootSignedInApp(page);

  const onboardingModal = page.locator('#onboarding-modal');

  await expect(onboardingModal).toHaveClass(/active/);

  await expect
    .poll(() =>
      page.evaluate(() => ({
        seen: profile.coaching?.onboardingSeen === true,
        completed: profile.coaching?.onboardingCompleted === true,
      }))
    )
    .toEqual({
      seen: true,
      completed: false,
    });

  await page.reload({ waitUntil: 'domcontentloaded' });
  await bootSignedInApp(page);

  await expect
    .poll(() =>
      page.evaluate(() => ({
        seen: profile.coaching?.onboardingSeen === true,
        active:
          document.getElementById('onboarding-modal')?.classList.contains(
            'active'
          ) === true,
      }))
    )
    .toEqual({
      seen: true,
      active: false,
    });
});
